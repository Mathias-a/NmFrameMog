...
Uses only numpy at runtime (no scipy dependency).

Source: challenge://astar-island/scoring (validated formula).
...
- Static cells (one-hot GT, entropy=0) are excluded from scoring.
- Ground truth is a soft distribution (from hundreds of sim runs).
...
def kl_divergence_per_cell(gt: Tensor3D, pred: Tensor3D) -> Tensor2D:
    ...
    Computes sum_c gt[c] * log(gt[c] / pred[c]) for each cell,
    only where gt > 0 (to avoid 0 * log(0)).
...
def entropy_per_cell(gt: Tensor3D) -> Tensor2D:
    ...
    H = -sum_c gt[c] * log(gt[c]), computed only where gt > 0.
...
def weighted_kl(gt: Tensor3D, pred: Tensor3D) -> float:
    ...
    Only dynamic cells (entropy > 0) contribute. Static cells
    (one-hot GT) are excluded because their entropy is zero.

    Returns:
        Weighted KL divergence. Lower is better. 0.0 = perfect.
        Returns 0.0 if all cells are static (no entropy).
    """
...
def competition_score(gt: Tensor3D, pred: Tensor3D) -> float:
    ...
    Returns:
        Score in [0, 100]. 100 = perfect prediction.
    """
...
def raw_kl_score(gt: Tensor3D, pred: Tensor3D) -> float:
    """Unweighted mean KL divergence (for debugging/comparison)."""
    kl = kl_divergence_per_cell(gt, pred)
    return float(np.sum(kl)) / kl.size
