...
class TestUniformStrategy:
    def test_output_shape(self, initial_state: InitialState) -> None:
        ...
        result = strategy.predict(initial_state, budget=Budget(total=50), base_seed=0)
    ...
    def test_deterministic_across_seeds(self, initial_state: InitialState) -> None:
        strategy = UniformStrategy()
        r1 = strategy.predict(initial_state, budget=Budget(total=50), base_seed=0)
        r2 = strategy.predict(initial_state, budget=Budget(total=50), base_seed=99)
    ...
    def test_dtype_is_float64(self, initial_state: InitialState) -> None:
        result = UniformStrategy().predict(initial_state, budget=Budget(total=50), base_seed=0)
        assert result.dtype == np.float64

    def test_respects_map_dimensions(self) -> None:
        grid = [[10] * 7 for _ in range(5)]
        state = InitialState(grid=grid, settlements=[])
        result = UniformStrategy().predict(state, budget=Budget(total=50), base_seed=0)
