> Source title: NorgesGruppen Data: Submission Format  
> Source URL: https://app.ainm.no/docs/norgesgruppen-data/submission  
> Imported: 2026-03-20
...
Your predictions must use the same `category_id` values. The imported examples and category listing indicate 357 classes (`0-356`, including `unknown_product`). When training YOLOv8 on this COCO data (with `nc=357`), the model learns the mapping and outputs the correct `category_id` during inference.
